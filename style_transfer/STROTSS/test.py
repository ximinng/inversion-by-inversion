...
from .utils import extract_regions
...
if __name__ == '__main__':
    ...
    if use_guidance_region:
        content_regions, style_regions = extract_regions(args.content_guidance, args.style_guidance)
    else:
        content_img, style_img = imageio.imread(args.content), imageio.imread(args.style)
        content_regions, style_regions = [np.ones(content_img.shape[:2], dtype=np.float32)], [
            np.ones(style_img.shape[:2], dtype=np.float32)]

    loss, canvas = run_style_transfer(args.content, args.style, content_weight,
