...
class ILVRDiffusion(GaussianDiffusion):
                ...
                if resizers is not None:
                    if i > range_t:
                        out["sample"] = out["sample"] - up(down(out["sample"])) + up(
                            down(self.q_sample(extra_kwargs["ref_img"], t, torch.randn(*shape, device=device)))
                        )
...
class SpacedDiffusion(ILVRDiffusion):
    """
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = ILVRDiffusion(**kwargs)  # pylint: disable=missing-kwoa
