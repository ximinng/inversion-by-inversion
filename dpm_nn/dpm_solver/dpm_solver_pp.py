...
class NoiseScheduleVP:
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == 'linear':
            ...
            return interpolate_fn(t.reshape((-1, 1)), self.t_discrete.clone().to(t.device),
                                  self.log_alpha_discrete.clone().to(t.device)).reshape((-1,))
...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1.,
                  time_input_type='1', total_N=1000, model_kwargs={}, is_deis=False):
...
class DPM_Solver:
        ...
        if self.predict_x0:
            ...
            if solver_type == 'dpm_solver':
                ...
                x_t = (
                        (sigma_t / sigma_s)[(...,) + (None,) * dims] * x
                        - (alpha_t * phi_1)[(...,) + (None,) * dims] * noise_s
                        + (1. / r1) * (alpha_t * ((torch.exp(-h) - 1.) / h + 1.))[(...,) + (None,) * dims] * (
                                noise_s1 - noise_s)
                )
            else:
            ...
            if solver_type == 'dpm_solver':
                ...
                x_t = (
                        torch.exp(log_alpha_t - log_alpha_s)[(...,) + (None,) * dims] * x
                        - (sigma_t * phi_1)[(...,) + (None,) * dims] * noise_s
                        - (1. / r1) * (sigma_t * ((torch.exp(h) - 1.) / h - 1.))[(...,) + (None,) * dims] * (
                                noise_s1 - noise_s)
                )
            else:
                raise ValueError("solver_type must be either dpm_solver or taylor, got {}".format(solver_type))
    ...
    def dpm_solver_third_update(self, x, s, t, r1=1. / 3., r2=2. / 3., noise_s=None, noise_s1=None, noise_s2=None,
                                return_noise=False, solver_type='dpm_solver'):
        """
    ...
    def dpm_solver_update(self, x, s, t, order, return_noise=False, solver_type='dpm_solver', r1=None, r2=None):
        ...
        if order == 1:
            ...
            return self.dpm_solver_third_update(x, s, t, return_noise=return_noise, solver_type=solver_type, r1=r1,
                                                r2=r2)
        else:
    ...
    def dpm_solver_adaptive(self, x, order, t_T, t_0, h_init=0.05, atol=0.0078, rtol=0.05, theta=0.9, t_err=1e-5,
        ...
        if order == 2:
            ...
            higher_update = lambda x, s, t, **kwargs: self.dpm_solver_second_update(x, s, t, r1=r1,
                                                                                    solver_type=solver_type, **kwargs)
        elif order == 3:
            r1, r2 = 1. / 3., 2. / 3.
            lower_update = lambda x, s, t: self.dpm_solver_second_update(x, s, t, r1=r1, return_noise=True,
                                                                         solver_type=solver_type)
            higher_update = lambda x, s, t, **kwargs: self.dpm_solver_third_update(x, s, t, r1=r1, r2=r2,
                                                                                   solver_type=solver_type, **kwargs)
        else:
        ...
        if method == 'adaptive':
            ...
            with torch.no_grad():
                ...
                for init_order in range(1, order):
                    ...
                    x = self.dpm_multistep_update(x, noise_prev_list, t_prev_list, vec_t, init_order,
                                                  solver_type=solver_type)
                    noise_prev_list.append(self.model_fn(x, vec_t))
                    t_prev_list.append(vec_t)
                for step in range(order, steps + 1):
                    vec_t = timesteps[step].expand(x.shape[0])
                    x = self.dpm_multistep_update(x, noise_prev_list, t_prev_list, vec_t, order,
                                                  solver_type=solver_type)
            ...
            orders, _ = self.get_time_steps_for_dpm_solver_fast(skip_type=skip_type, t_T=t_T, t_0=t_0, steps=steps,
