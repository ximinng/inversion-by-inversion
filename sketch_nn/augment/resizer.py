...
class Resizer(nn.Module):
    def __init__(self, in_shape, scale_factor=None, output_shape=None, kernel='linear', antialiasing=True):
        ...
        # print(scale_factor,output_shape):[1,1,0.3125,0.03125],[B,3,8,8]

        # Choose interpolation methods, each methods has the matching kernel size
        method, kernel_width = {
            "cubic": (cubic, 4.0),
            ...
            "box": (box, 1.0),
            "linear": (linear, 2.0),
            None: (cubic, 4.0)  # set default interpolation methods as cubic
        }.get(kernel)
        # print(methods,kernel_width)
        ...
        # print(sorted_dims)
        self.sorted_dims = [int(dim) for dim in sorted_dims if scale_factor[dim] != 1]  # the dim needed to be scale
        # print(self.sorted_dims)
    ...
    def forward(self, in_tensor):
        ...
        for dim, fov, w in zip(self.sorted_dims, self.field_of_view, self.weights):
            # print(dim)
            # print(fov.shape)#kernel_width*out_length
            # print(w.shape)#kernel*out_length*1*1
    ...
    def contributions(self, in_length, out_length, scale, kernel, kernel_width, antialiasing):
        ...
        # 'weights' based on the interpolation methods and the distance of the sub-pixel location from the pixel centers
        # around it. This is only done for one dimension of the image.

        # When anti-aliasing is activated (default and only for downscaling) the receptive field is stretched to size of
        # 1/sf. this means filtering is more 'low-pass filter'.

        # print(in_length,out_length,scale,kernel,kernel_width,antialiasing):256,8,0.03125,linear,2.0,true
        fixed_kernel = (lambda arg: scale * kernel(scale * arg)) if antialiasing else kernel
        kernel_width *= 1.0 / scale if antialiasing else 1.0
        # print(kernel_width)

        # These are the coordinates of the output image
        out_coordinates = np.arange(1, out_length + 1)  # [1,2,……,8]
        # print(out_coordinates)

        # since both scale-factor and output size can be provided simulatneously, perserving the center of the image requires shifting
        # the output coordinates. the deviation is because out_length doesn't necesary equal in_length*scale.
        # to keep the center we need to subtract half of this deivation so that we get equal margins for boths sides and center is preserved.
        shifted_out_coordinates = out_coordinates - (
                out_length - in_length * scale) / 2  # equalivent to out_coordinates
        # print(shifted_out_coordinates)
        ...
        # print(field_of_view.shape)#out_length*expanded_kernel_width

        # Assign weight to each pixel in the field of view. A matrix whos horizontal dim is the output pixels and the
        # vertical dim is a list of weights matching to the pixel in the field of view (that are specified in
        # 'field_of_view')
        weights = fixed_kernel(1.0 * np.expand_dims(match_coordinates, axis=1) - field_of_view - 1)
        # print(weights.shape)#out_length*expanded_kernel_width
