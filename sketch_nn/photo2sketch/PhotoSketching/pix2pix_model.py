...
from .networks import define_G, define_D, GANLoss, get_scheduler, print_network
...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = define_G(opt.input_nc, opt.output_nc, opt.ngf,
                             opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type)

        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD = define_D(opt.input_nc + opt.output_nc, opt.ndf,
